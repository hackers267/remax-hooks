...
import usePrevious, { compareFunction as comFunc } from '../index';
...
describe('usePrevious', () => {
  ...
  it('should work will with undefined', () => {
    const hooks = renderHook(({ value }) => usePrevious(value), {
      initialProps: {
        value: undefined as undefined | number,
      },
    });

    expect(hooks.result.current).toBeUndefined();

    hooks.rerender({ value: 1 });
    expect(hooks.result.current).toBeUndefined();

    hooks.rerender({ value: undefined });
    expect(hooks.result.current).toBe(1);

    hooks.rerender({ value: 2 });
    expect(hooks.result.current).toBeUndefined();
  });
  ...
  it('should receive a predicate as a second parameter that will compare prev and current', () => {
    ...
    hook.rerender({ val: obj2, cmp: predicate });
    expect(hook.result.current).toBe(obj3);
  });
});
